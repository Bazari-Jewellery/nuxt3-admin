...
async function createSection() {
  ...
  const data_ = await useCybandyClient().customMethods.faqSection.create({ name: section.value.name, description: section.value.description })
...
<template>
  <ModalTitleButton v-model="modal" @send="createSection" title="Add Section" button-confirm-label="Add Section"
    width="min-w-full sm:min-w-[500px] md:min-w-[650px]">
