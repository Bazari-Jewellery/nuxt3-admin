...
async function send_fulfillment(){
  ...
  const {order:_order_res} = await useCybandyClient().admin.orders.createFulfillment(order.value.id, {
    items:_items,
          ...
          <UCard :ui="{divide:''}">
            <template #header>
              <div class="w-full flex items-center justify-between">
                <UIcon @click="cancel" name="i-heroicons-x-mark-20-solid"
            ...
            <div class="space-y-2">
                <div class="w-full flex gap-4">
                  ...
                  <span class="text-base text-black dark:text-white">Items to be fulfilled</span>
                </div>
                <div v-if="order.fulfillments" v-for="ff of order.fulfillments"
                  ...
                  <span class="py-2">Select the number of items that you wish to fulfill</span>
                  <div v-for="it_ff of ff.items" class="flex justify-between">
                    ...
                    <UInput :min="0" :max="quantity_input_boxes[it_ff.item_id].to_fulfil" v-model="quantity_input_boxes[it_ff.item_id].to_fulfil"
                    :ui="{ icon: { trailing: { pointer: '' } } }"
                    >
                      
                      <template #trailing>
                        <span class="flex items-center gap-1">
                          <span>/{{ quantity_input_boxes[it_ff.item_id].quantity }}</span>
                          <span class="flex items-center gap-2">
                            <UIcon @click="()=>minus(it_ff.item_id)" name="i-heroicons-minus" class="w-4 h-4 cursor-pointer" />
                            <UIcon @click="()=>plus(it_ff.item_id)" name="i-heroicons-plus" class="w-4 h-4 cursor-pointer" />
                          </span>
                        </span>
                      </template>
                    </UInput>
